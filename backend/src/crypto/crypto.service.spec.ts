...
describe('CryptoService', () => {
  ...
  it('matches Noir hash_1', () => {
    expect(service.hash1(7n)).toBe(7061949393491957813657776856458368574501817871421526214197139795307327923534n);
  });
  ...
  it('computes hash_2 of the zero leaf recursively', () => {
    ...
    expect(z0).toBe(service.computeCreditLeafHash({ creditId: '', vintage: 0, volume: 0, methodologyCode: 0 }));
  ...
  it('creditIdToField is deterministic and field-safe', () => {
    ...
    expect(f).toBeLessThan(service.fieldOrder);
  ...
  it('computes nullifiers and commitments consistently', () => {
    ...
    expect(service.computeVolumeCommitment(3000n, 99n)).toMatch(/^0x[0-9a-f]{64}$/);
  });
});
